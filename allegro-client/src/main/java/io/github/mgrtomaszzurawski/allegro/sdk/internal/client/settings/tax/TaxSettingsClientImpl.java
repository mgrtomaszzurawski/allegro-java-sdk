...
public final class TaxSettingsClientImpl {
    ...
    /** Spec query name for the optional country filter (repeated per country). */
    ...
    /**
     * Read the tax options available for the given category. When
     * {@code countryCodes} is non-empty the result is narrowed to those countries;
     * an empty list returns the settings for every country.
     */
    public TaxSettings taxSettings(String categoryId, List<String> countryCodes) {
        ...
        Query query = Query.create().add(PARAM_CATEGORY_ID, categoryId);
        if (countryCodes != null && !countryCodes.isEmpty()) {
            query.addAll(PARAM_COUNTRY_CODE, countryCodes);
        }
