...
/**
 * How the delivery cost of a multi-item order is combined when several items ship
 * with the same method (the seller's delivery-settings join policy).
 *
...
public enum JoinStrategy {
    ...
    /** Charge the cheapest of the joined items' delivery rates (spec value {@code MIN}). */
    MIN,
    ...
    public static JoinStrategy fromWire(@Nullable String raw) {
        if (raw == null) {
            return UNKNOWN;
        }
        try {
